...
export default function HotelOnboardingStep9Page() {
  const { t } = useLanguage();
  const navigate = useNavigate();
  const location = useLocation();
  const copy = useMemo(() => t.onboardingHotelStep9Wip, [t.onboardingHotelStep9Wip]);
  ...
  return (
    ...
    >
      ...
      <main className="px-6 max-w-[640px] mx-auto pb-32 pt-10">
        <form id="hotel-step9-wip-form" onSubmit={handleSubmit}>
      ...
      <OnboardingBottomNav
        ...
        onBack={() => navigate("/onboarding/hotel/step-8", { state: location.state })}
        formId="hotel-step9-wip-form"
      />
    </motion.div>
  );
}
